...
class convAE(nn.Module):
    def __init__(self, input_dim, encoded_dim, output_dim, key_dim, memory_size):
        super(convAE, self).__init__()
        self.encoder = Encoder(input_dim, encoded_dim)
        self.decoder = Decoder(encoded_dim, output_dim)
    ...
    def forward(self, x, train=True):
        ...
        if train:
            updated_query, keys, softmax_score_query, softmax_score_memory, gathering_loss, spreading_loss, mem_reconstruction_loss = self.memory(
                fea)
            output = self.decoder(updated_query)
...
if __name__ == '__main__':
    ...
    file_path = 'your_deep_features_tain.npy'
    ...
    n_clusters = 500
    model = convAE(input_dim=512, encoded_dim=34, output_dim=512, key_dim=34, memory_size=n_clusters)
    model.encoder.load_state_dict(torch.load('state/ae_encoder/encoder_best_epoch.pth'))
    model.decoder.load_state_dict(torch.load('state/ae_encoder/decoder_best_epoch.pth'))

    cluster_centers_tensor = torch.load('state/clusters/cluster_centers_tensor.pth')
    with torch.no_grad():
